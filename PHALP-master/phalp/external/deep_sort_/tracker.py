...
from phalp.visualize.kalman_filter import AdaptiveKalmanFilter
...
class Tracker:
    ...
    def update(self, detections, frame_t, image_name, shot):
        ...
        for track_idx, detection_idx in matches:
            if self.cfg.post_process.apply_kalman:
                self.tracks[track_idx].update_with_kalman(detections[detection_idx], detection_idx, shot)
            else:
    ...
    def update_with_kalman(self, detection, detection_id, shot):
        ...
        print(f"原始位置: {detection.detection_data['loca']}")
        # print(f"卡尔曼滤波后位置: {kalman_detection.detection_data['loca']}")