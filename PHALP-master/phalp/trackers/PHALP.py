...
class PHALP(nn.Module):
    ...
    def track(self):
        ...
        try: 
            start_frame = 0 if self.cfg.phalp.start_frame == -1 else self.cfg.phalp.start_frame
            end_frame   = len(list_of_frames) if self.cfg.phalp.end_frame == -1 else self.cfg.phalp.end_frame
            list_of_frames = list_of_frames[start_frame:end_frame]
            ...
            for t_, frame_name in progress_bar(enumerate(list_of_frames), description="Tracking : " + self.cfg.video_seq, total=len(list_of_frames), disable=False):
                ...
                self.cfg.phalp.shot       = 1 if t_ in list_of_shots else 0 #是否刚好切换场景的时间点
