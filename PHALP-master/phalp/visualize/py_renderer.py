...
class Renderer:
    ...
    def add_joints_as_spheres(self, scene, joints, radius=0.01, color=(1.0, 0.0, 0.0, 1.0)):
        ...
        for joint in joints[0]:  # joints shape: (1, 49, 3)
    ...
    def __call__(self, vertices, focal_length=5000, baseColorFactors=[(1.0, 1.0, 0.9, 1.0)]):
        ...
        for i_, verts in enumerate(vertices):
            ...
            scene.add(mesh, 'mesh')
            # 如果你想可控显示球体，可以加个参数控制
            if hasattr(self, "joints_to_render") and self.joints_to_render is not None:
                self.add_joints_as_spheres(scene, self.joints_to_render)
