...
class Visualizer(nn.Module):
    ...
    def visualize_keypoints(self, image,img_height, img_width, keypoints, color, radius=20, thickness=10):
        """
        Visualize 2D keypoints on the image.

        Args:
            image: tensor, shape (1, 3, H, W)
            keypoints: list or tensor, [x1, y1, x2, y2, ..., xk, yk]
            color: (R, G, B), each in [0,255] or [0,1]
            radius: radius of keypoints
            thickness: thickness of circles
        """
        ...
        # Convert to numpy
        img = image[[2,1,0], :, :]  # RGB to BGR for OpenCV
        ndarr = img.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
        
        cv_ndarr = ndarr  # No need to resize
        cv2.imwrite("before_image.png", cv_ndarr)

        # Handle color if in [0,1]
        if max(color) <= 1.0:
        ...
        for i in range(0, len(keypoints), 2):
            if i + 1 < len(keypoints): 
                x, y = keypoints[i] *img_width, keypoints[i+1]*img_height
                print(x,y)
                cv2.circle(cv_ndarr, (int(x), int(y)), radius, color, thickness)
        cv2.imwrite("output_image.png", cv_ndarr)  # 保存图像到本地
    ...
    def render_video(self, final_visuals_dic):
        ...
        track_2d_joints =    final_visuals_dic["2d_joints"] 
        # print(track_2d_joints)
        ...
        if PANEL_TEXTURE:
            ...
            # import pdb;pdb.set_trace()
            color = (0, 255, 0)  # 绿色
            # rendered_image_final = self.visualize_keypoints(rendered_image_final, img_height, img_width,track_2d_joints, color)
