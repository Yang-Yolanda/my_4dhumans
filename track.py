...
# print("CUDA_VISIBLE_DEVICES = 0", os.environ.get("CUDA_VISIBLE_DEVICES"))
...
class HMR2023TextureSampler(HMR2Predictor):
    def __init__(self, cfg) -> None:
        ...
        self.img_size = 128         #self.cfg.MODEL.IMAGE_SIZE
        self.focal_length = 1376.   #self.cfg.EXTRA.FOCAL_LENGTH
...
def main(cfg: DictConfig) -> Optional[float]:
    ...
    # 运行跟踪
    result = phalp_tracker.track()
    
    # 如果track()返回了结果，打印保存路径
    if isinstance(result, tuple) and len(result) == 2:
        final_visuals_dic, pkl_path = result
        print(f"跟踪完成，结果保存到: {pkl_path}")
        return pkl_path
    else:
        print("跟踪完成")
        return None
    # print(phalp_tracker.cfg.post_process.enable)
    # if phalp_tracker.cfg.post_process.enable:
    #     final_visuals_dic_new = phalp_tracker.postprocessor.post_process(final_visuals_dic)
    # # 3. 离线渲染阶段
    # if phalp_tracker.cfg.render.enable:
    #     save_pkl_path = pkl_path
    #     save_video_path = pkl_path.replace('.pkl', '.mp4')
    #     phalp_tracker.postprocessor.run_renderer(pkl_path )
    #     print(f"渲染完成，视频保存至: {save_video_path}")
