...
class SMPLTransformerDecoderHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        transformer_args = dict(
            num_tokens=1,
            token_dim=(npose + 13 + 3) if self.input_is_mean_shape else 1,
            dim=1024,
        )
    ...
    def forward(self, x, **kwargs):
        ...
        for i in range(self.cfg.MODEL.SMPL_HEAD.get('IEF_ITERS', 1)):
            ...
            if self.input_is_mean_shape:
                # Pad to 160: 144 (pose) + 10 (betas) + 3 (cam) + 3 (padding)
                token = torch.cat([pred_body_pose, pred_betas, pred_cam, torch.zeros(batch_size, 3).to(pred_cam.device)], dim=1)[:,None,:]
            else:
                token = torch.zeros(batch_size, 1, self.transformer.to_token_embedding.in_features).to(x.device)
