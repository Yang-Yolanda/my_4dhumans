...
class HMR2DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if self.train_dataset == None:
            ...
            if mocap_key and mocap_key in self.dataset_cfg:
                ...
                if mocap_file and os.path.exists(mocap_file):
                    self.mocap_dataset = MoCapDataset(**to_lower(self.dataset_cfg[mocap_key]))
    ...
    def train_dataloader(self) -> Dict:
        ...
        """
        # NOTE: pin_memory=False required for WebDataset - it uses its own
        # IPC/shared-memory model that conflicts with PyTorch's pin_memory
        # and causes ConnectionResetError in multiprocessing.resource_sharer.
        num_workers = self.cfg.GENERAL.NUM_WORKERS
        train_dataloader = torch.utils.data.DataLoader(
            self.train_dataset, self.cfg.TRAIN.BATCH_SIZE,
            drop_last=True,
            num_workers=num_workers,
            prefetch_factor=self.cfg.GENERAL.PREFETCH_FACTOR if num_workers > 0 else None,
            persistent_workers=(num_workers > 0),
            pin_memory=False,  # DO NOT enable: WebDataset IPC conflict
        )
        if self.mocap_dataset is not None:
            mocap_dataloader = torch.utils.data.DataLoader(
                ...
                shuffle=True, drop_last=True, num_workers=1, pin_memory=False,
            )
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        val_dataloader = torch.utils.data.DataLoader(
            self.val_dataset, self.cfg.TRAIN.BATCH_SIZE,
            drop_last=True,
            num_workers=num_workers,
            persistent_workers=(num_workers > 0),
            pin_memory=False,  # DO NOT enable: WebDataset IPC conflict
